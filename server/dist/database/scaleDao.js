...
class ScaleDao {
     ...
     */
    async getWeightSessions(filters = {}) {
        try {
            const { startDate, endDate, limit = scale_1.DEFAULT_QUERY_LIMIT, offset = 0 } = filters;
            const queryLimit = Math.min(limit, scale_1.MAX_QUERY_LIMIT);
            let query = `
        SELECT 
          image_date as date,
          health_score,
      ...
      `;
            ...
            throw new scale_1.ScaleDataError('Failed to fetch weight sessions');
        }
    }
    /**
     * Get weight session summary with before/after comparison
     */
    async getWeightSessionSummary(filters = {}) {
        try {
            const { startDate, endDate, limit = scale_1.DEFAULT_QUERY_LIMIT, offset = 0 } = filters;
            const queryLimit = Math.min(limit, scale_1.MAX_QUERY_LIMIT);
            let query = `
      ...
      `;
            ...
            client.release();
            return result.rows.map(row => {
                const weightChange = (0, scale_1.calculateChange)(row.weight_after, row.weight_before);
                const bodyFatChange = (0, scale_1.calculateChange)(row.body_fat_percentage_after, row.body_fat_percentage_before);
                return {
                    date: row.date.toISOString().split('T')[0],
                    health_score: row.health_score,
            ...
            throw new scale_1.ScaleDataError('Failed to fetch weight session summary');
        }
    }
    /**
    ...
    async getLatestHealthSnapshot() {
      ...
      `;
            ...
            throw new scale_1.ScaleDataError('Failed to fetch latest health snapshot');
        }
    }
    /**
     * Calculate weight delta for specified time period
     */
    async getWeightDelta(days) {
        try {
            const query = `
        WITH recent_data AS (
          SELECT 
      ...
      `;
            ...
            client.release();
            if (result.rows.length === 0 || !result.rows[0].current_weight)
                return null;
            const row = result.rows[0];
            const change = (0, scale_1.calculateChange)(row.current_weight, row.previous_weight);
            const changePercentage = (0, scale_1.calculatePercentageChange)(row.current_weight, row.previous_weight);
            return {
                metric: scale_1.HEALTH_METRICS.WEIGHT,
                current_value: row.current_weight,
                previous_value: row.previous_weight,
                change_amount: change,
                change_percentage: changePercentage,
                trend_direction: (0, scale_1.getTrendDirection)(change),
                days_analyzed: Math.min(days, row.days_analyzed)
            };
        }
        catch (error) {
            console.error('❌ Error calculating weight delta:', error);
            throw new scale_1.ScaleDataError('Failed to calculate weight delta');
        }
    }
    /**
...
exports.ScaleDao = ScaleDao;
//# sourceMappingURL=scaleDao.js.map