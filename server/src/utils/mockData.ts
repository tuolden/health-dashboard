...
// Steps Mock Data
export const generateStepsData = (timeRange?: TimeRange): StepsHistory => {
...
export const generateWaterIntakeData = (date?: Date): WaterIntakeData => {
...
// Weight Mock Data
export const generateWeightData = (timeRange?: TimeRange): WeightHistory => {
  ...
  if (weeklyChange > 0.5) trend = 'increasing'
  else if (weeklyChange < -0.5) trend = 'decreasing'
...
export const generateHeartRateData = (timeRange?: TimeRange): HeartRateData => {
  ...
  const history = Array.from({ length: 24 }, (_, i) => ({
    ...
    type: i < 8 ? 'resting' : i < 16 ? 'active' : 'recovery' as const
  }))
...
// Nutrition Mock Data
export const generateNutritionData = (date?: Date): NutritionData => {
  const targetDate = date || new Date()
  ...
  const score = Math.min(100, Math.round(
    (totals.protein / goals.protein * 25) +
...
// Sleep Mock Data
export const generateSleepData = (timeRange?: TimeRange): SleepHistory => {
  const generateSleepEntry = (daysAgo: number) => {
...
// Activity Mock Data
export const generateActivityData = (date?: Date): ActivityData => {
  const targetDate = date || new Date()
