...
import { widgetRegistry, getAffectedWidgets, isValidDataset } from '../graphql/widgetRegistry'
import { publishDatasetRefresh, publishWebhookReceived, publishWidgetUpdate } from '../utils/pubsub'
 ...
 */
const processWebhookPayload = async (payload: WebhookPayload, pubsub: PubSub): Promise<void> => {
  ...
  for (const widgetType of affectedWidgets) {
    await publishWidgetUpdate(widgetType, {
      source,
      dataset,
      data,
  ...
  await publishWebhookReceived(source, payload)
 ...
 */
const handleWebhook = async (req: Request, res: Response, pubsub: PubSub): Promise<void> => {
  try {
    // Rate limiting
    try {
 ...
 */
export const setupWebhooks = (app: Express, pubsub: PubSub): void => {
