...
import { ScaleQueryFilters, ScaleApiResponse, ScaleDataError } from '../types/scale'
...
router.get('/health', async (_req: Request, res: Response) => {
  try {
    ...
    const response: ScaleApiResponse<any> = {
      success: true,
      data: {
    ...
    const response: ScaleApiResponse<null> = {
      success: false,
 ...
 */
router.get('/weight-sessions/summary', async (req: Request, res: Response) => {
  try {
    const {
      startDate,
      endDate,
    ...
    const filters: ScaleQueryFilters = {
    ...
    const response: ScaleApiResponse<typeof sessions> = {
      success: true,
      data: sessions,
      timestamp: new Date().toISOString(),
    ...
    const response: ScaleApiResponse<null> = {
      success: false,
      error: error instanceof ScaleDataError ? error.message : 'Failed to fetch weight sessions',
      timestamp: new Date().toISOString()
    }
    res.status(500).json(response)
  }
 ...
 */
router.get('/weight-sessions', async (req: Request, res: Response) => {
  try {
    const {
      startDate,
      endDate,
 ...
 */
router.get('/health-snapshot', async (_req: Request, res: Response) => {
  try {
    const snapshot = await scaleDao.getLatestHealthSnapshot()

    const response: ScaleApiResponse<typeof snapshot> = {
      success: true,
      data: snapshot,
      timestamp: new Date().toISOString()
    }
    ...
    const response: ScaleApiResponse<null> = {
      success: false,
      error: error instanceof ScaleDataError ? error.message : 'Failed to fetch health snapshot',
      timestamp: new Date().toISOString()
    }
    res.status(500).json(response)
  }
 ...
 */
router.get('/weight-delta', async (req: Request, res: Response) => {
  try {
    ...
    const response: ScaleApiResponse<typeof delta> = {
      success: true,
      data: delta,
      timestamp: new Date().toISOString()
    }
    ...
    const response: ScaleApiResponse<null> = {
      success: false,
      error: error instanceof ScaleDataError ? error.message : 'Failed to calculate weight delta',
      timestamp: new Date().toISOString()
    }
...
router.get('/raw', async (req: Request, res: Response) => {
  try {
    ...
    const queryLimit = Math.min(parseInt(limit as string), 50) // Max 50 records
    ...
    const response: ScaleApiResponse<typeof result.rows> = {
      success: true,
      data: result.rows,
      timestamp: new Date().toISOString(),
